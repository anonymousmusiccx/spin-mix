...
export const TempoFader = ({ value, onChange }: { value: number; onChange: (v: number) => void }) => {
  return (
    <div className="tempo-fader-container flex flex-col items-center h-48 py-2">
      <span className="text-xs font-semibold text-gray-300 mb-1">TEMPO</span>
      <input
        type="range"
        min="-16"
        max="16"
        ...
        className="vertical-slider h-36 appearance-none bg-gray-700 rounded-lg outline-none cursor-pointer"
        style={{ writingMode: 'bt-lr', WebkitAppearance: 'slider-vertical' }}
      />
      <span className="text-xs text-cyan-400 mt-1">{value > 0 ? `+${value}%` : `${value}%`}</span>
    </div>
  );
};
