...
// src/components/DoubleFxUnit.tsx
import React, { useState } from 'react';

const RECOMMENDED_FX = [
  'Filter (HPF/LPF)', 'Reverb', 'Echo', 'Flanger', 
  'Phaser', 'Pitch Shift', 'Noise Generator', 'Beat Grid Break'
];

export const DoubleFxUnit = ({ deckId }: { deckId: 'A' | 'B' }) => {
  const [fx1, setFx1] = useState('Echo');
  const [fx2, setFx2] = useState('Filter (HPF/LPF)');
  const [combineFx, setCombineFx] = useState(true);

  return (
    <div className="double-fx-panel controller-panel p-3">
      <div className="flex justify-between items-center mb-2">
        <h4 className="text-xs font-bold text-gray-300">DECK {deckId} FX UNIT</h4>
        <label className="text-xs flex items-center cursor-pointer gap-1 text-cyan-400">
          <input 
            type="checkbox" 
            checked={combineFx} 
            onChange={(e) => setCombineFx(e.target.checked)} 
          />
          Combine Dual FX
        </label>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {/* FX Slot 1 */}
        <div className="fx-slot bg-gray-800 p-2 rounded">
          <select 
            value={fx1} 
            onChange={(e) => setFx1(e.target.value)}
            className="w-full bg-gray-900 text-xs text-white p-1 rounded"
          >
            {RECOMMENDED_FX.map(fx => <option key={fx} value={fx}>{fx}</option>)}
          </select>
        </div>

        {/* FX Slot 2 */}
        <div className={`fx-slot bg-gray-800 p-2 rounded ${!combineFx ? 'opacity-40 pointer-events-none' : ''}`}>
          <select 
            value={fx2} 
